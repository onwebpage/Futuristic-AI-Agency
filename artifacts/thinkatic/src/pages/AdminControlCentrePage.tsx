        ...
        <nav className="space-y-1">
          {navItems.map(({ key, label, icon: Icon }) => (
            <button
              ...
              className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-medium transition ${activeTab === key ? "bg-primary/5 text-primary" : "text-slate-600 hover:bg-slate-100"}`}
            >
      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8">
        <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-sm">
            <Search className="h-4 w-4 text-slate-500" />
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Global search" className="w-52 bg-transparent text-sm text-slate-700 outline-none placeholder:text-slate-500" />
          </div>
        </div>
        ...
        <nav className="mb-5 flex gap-2 overflow-x-auto pb-1 lg:hidden" aria-label="Admin modules">
          {navItems.map(({ key, label }) => (
            <button
              key={key}
              onClick={() => setActiveTab(key)}
              className={`shrink-0 rounded-xl px-3 py-2 text-xs font-semibold ${activeTab === key ? "bg-primary text-white" : "border border-slate-200 bg-white text-slate-600"}`}
            >
              {label}
            </button>
          ))}
